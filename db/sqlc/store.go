...
import (
	...
	"database/sql"
	"fmt"
	_ "github.com/golang/mock/mockgen/model" // to allow mockgen to work properly
)
...
// Store : We are adding this interface so that we can use a mock DB in the api server test
// In the sqlc.yaml we will change the emit_interface to true so that it can generate an interface
// for our mock
// That's why we need to this interface
// We are going to define a list of actions that the Store interface can do
// which is basically all functions of the Queries struct and one more for the transaction
// First, we are adding the TransferTx function signature
type Store interface {
	Querier // this is the interface generated by sqlc
	TransferTx(ctx context.Context, arg TransferTxParams) (TransferTxResult, error)
}

// SQLStore provides all functions to execute db queries and transactions
// this will be the real implementation of the Store interface that talks to a SQL database (PostgresSQL)
...
// New store now no longer return a pointer but just a Store interface
